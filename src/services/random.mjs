// Service exposing random data generators (adapted from main.js)
// Uses global fetch (Node 18+) and falls back to a default API key if not provided
const RANDOMMER_API_KEY = process.env.RANDOMMER_API_KEY || '9972b6e41a37489890db8a6f25933ee3';
...
export async function getAll() {
  const [user, phoneNumber, iban, creditCard, randomName, quote, joke] = await Promise.all([
    getRandomUser(),
    getPhoneNumber(),
    getIban(),
    getCreditCard(),
  ...
  return { user, phoneNumber, iban, creditCard, randomName, quote, joke };
}
